# The Telltale Inspector (Linux Ver.)
...
The original version can only run on Windows and even so it does not have necessary files to build. But I don't want to install such a huge Visual Studio in my Wine prefix...... So I took some time and modified the (a bit windows-ish) code to make it compile on Linux. Currently it can be only compiled within Codeblocks.

Also, because the original version use `MessageBoxA` to ask user some questions and I don't know how to invoke GTK to do the same job, I replaced them with just printing the message in the terminal, and if it's asking a question, give it a default value. So if you want to adjust its behaviour, you need to recompile the source code. I guess this may be easy for you if you've already been reading this... :)
